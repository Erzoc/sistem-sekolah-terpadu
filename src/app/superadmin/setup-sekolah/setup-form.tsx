...
export default function SetupSekolahForm({ initialTenant }: any) {
  const [formData, setFormData] = useState({
    namaSekolah: initialTenant?.schoolName || 'YPI NH',
    npsn: initialTenant?.npsn || '802836',
    nss: initialTenant?.nss || '',
    jenjang: initialTenant?.level || 'SMA',
    status: initialTenant?.status || 'swasta',
    akreditasi: initialTenant?.accreditation || 'A',
    provinsi: initialTenant?.province || 'Sumatera Utara',
    kabupaten: initialTenant?.city || 'Medan',
    kecamatan: initialTenant?.district || '',
    kelurahan: initialTenant?.village || '',
    alamat: initialTenant?.address || 'Jl Pertahanan SUMATERA UTARA',
    kodePos: initialTenant?.postalCode || '',
    telepon: initialTenant?.phone || '021 6536782',
    email: initialTenant?.email || 'infoypi@sch.id',
    website: initialTenant?.website || '',
    fax: initialTenant?.fax || '',
    namaKepsek: '',
    nipKepsek: '',
    tahunBerdiri: initialTenant?.foundedYear || '',
    visi: initialTenant?.vision || '',
    misi: initialTenant?.mission || '',
  });
  ...
  const handleSave = async () => {
    if (!formData.namaSekolah || !formData.npsn || !formData.provinsi) {
      setErrorMsg('Nama Sekolah, NPSN, dan Provinsi wajib diisi');
      setSaveStatus('error');
      return;
    }
    ...
    try {
      const response = await fetch('/api/superadmin/school-profile', {
        method: 'POST',
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-500 mt-1">Nomor Pokok Sekolah Nasional</p>
                </div>

                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">NSS</label>
                  <div className="relative">
                    <Hash className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <input
                      type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Akreditasi</label>
                  <select
                    name="akreditasi"
          ...
          <div className="bg-white rounded-lg border">
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-2">Kelurahan/Desa</label>
                  <input
                    type="text"
                    name="kelurahan"
          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-lg border">
            ...
            {saveStatus === 'error' && (
              <div className="flex items-center gap-2 text-red-600">
                ...
                <span className="font-medium">{errorMsg || 'Gagal menyimpan data'}</span>
              </div>
            )}
        ...
        <div className="space-y-6">
            ...
            <div className="bg-gradient-to-r from-slate-50 to-slate-100 px-5 py-4 border-b">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-600">Data yang akan disimpan</p>
                </div>
              </div>
            </div>
            ...
            <div className="p-5 space-y-4">
              <div className="bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 rounded-lg p-4 border border-blue-100">
                ...
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  ...
                  {formData.status && (
                    <span className="px-2 py-1 bg-slate-500 text-white text-xs font-medium rounded capitalize">
                      {formData.status}
                    </span>
                  )}
                  {formData.akreditasi && (
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="bg-slate-50 rounded-lg p-3 border border-slate-200">
                  <p className="text-xs text-slate-600 mb-1">NPSN</p>
                ...
                {formData.tahunBerdiri && (
                  <div className="bg-slate-50 rounded-lg p-3 border border-slate-200 col-span-2">
                    <p className="text-xs text-slate-600 mb-1">Tahun Berdiri</p>
              ...
              <div className="bg-green-50 rounded-lg p-4 border border-green-100">
                <p className="text-xs text-green-700 font-medium mb-2">Lokasi</p>
                <p className="text-sm font-semibold text-green-900">
                  {formData.kabupaten}, {formData.provinsi}
                </p>
                {formData.alamat && (
                  <p className="text-xs text-green-700 mt-2">{formData.alamat}</p>
                )}
              </div>
              ...
              <div className="flex items-center justify-center gap-2 py-3 bg-green-50 rounded-lg border border-green-200">
                ...
                <span className="text-sm font-medium text-green-700">Siap Disimpan</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl border border-blue-100 p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                <p className="font-semibold text-blue-900 text-sm mb-2">Panduan</p>
                <ul className="text-xs text-blue-800 space-y-1">
                  <li>• Field (*) wajib diisi</li>
                  <li>• NPSN harus 8 digit</li>
                  <li>• Sesuaikan dengan Dapodik</li>
                </ul>
              </div>
            </div>
          </div>
